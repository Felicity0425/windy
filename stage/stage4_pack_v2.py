...
from stage.reconstruct_utils_v2 import _reconstruct_wind_field
...
TEMPORAL_BG_BLEND = float(os.environ.get("WIND_RECON_TEMPORAL_BG_BLEND", "0.36"))
...
STAGE4_ENABLE_PRIMARY_ANCHOR_FALLBACK = os.environ.get("WIND_STAGE4_ENABLE_PRIMARY_ANCHOR_FALLBACK", "1") not in ("0", "false", "False")
PRIMARY_ANCHOR_FALLBACK_RATIO = float(os.environ.get("WIND_STAGE4_PRIMARY_ANCHOR_FALLBACK_RATIO", "0.08"))
PRIMARY_ANCHOR_FALLBACK_MAX = int(os.environ.get("WIND_STAGE4_PRIMARY_ANCHOR_FALLBACK_MAX", "8"))
PRIMARY_ANCHOR_FALLBACK_WEIGHT = float(os.environ.get("WIND_STAGE4_PRIMARY_ANCHOR_FALLBACK_WEIGHT", "0.18"))
...
DIRECT_SOURCE_SOFT_SPEED_CAP_MS = float(os.environ.get("WIND_DIRECT_SOURCE_SOFT_SPEED_CAP_MS", "120.0"))
DIRECT_SOURCE_SOFT_SPEED_PENALTY = float(os.environ.get("WIND_DIRECT_SOURCE_SOFT_SPEED_PENALTY", "0.35"))
DIRECT_ANCHOR_FORCE_DIFF_MS = float(os.environ.get("WIND_DIRECT_ANCHOR_FORCE_DIFF_MS", "8.0"))
DIRECT_ANCHOR_ZERO_SPEED_MS = float(os.environ.get("WIND_DIRECT_ANCHOR_ZERO_SPEED_MS", "1.0"))
RECON_SUPPORT_CONF_BOOST = float(os.environ.get("WIND_RECON_SUPPORT_CONF_BOOST", "0.01"))
RECON_TEMPORAL_CONF_BOOST = float(os.environ.get("WIND_RECON_TEMPORAL_CONF_BOOST", "0.03"))
RECON_INDIRECT_CONF_BOOST = float(os.environ.get("WIND_RECON_INDIRECT_CONF_BOOST", "0.03"))
RECON_RELAX_KEEP_RATIO = float(os.environ.get("WIND_RECON_RELAX_KEEP_RATIO", "0.95"))
RECON_ENABLE_CONF_BOOSTS = os.environ.get("WIND_RECON_ENABLE_CONF_BOOSTS", "0") not in ("0", "false", "False")
STAGE4_FAST_MODE = os.environ.get("WIND_STAGE4_FAST_MODE", "0") not in ("0", "false", "False")
STAGE4_FAST_SKIP_DENSE_AUX = os.environ.get("WIND_STAGE4_FAST_SKIP_DENSE_AUX", "1") not in ("0", "false", "False")
STAGE4_FAST_SKIP_POST = os.environ.get("WIND_STAGE4_FAST_SKIP_POST", "1") not in ("0", "false", "False")
STAGE4_SAVE_COMPRESSED = os.environ.get("WIND_STAGE4_SAVE_COMPRESSED", "1") not in ("0", "false", "False")
STAGE4_OUTPUT_PROFILE = os.environ.get(
    "WIND_STAGE4_OUTPUT_PROFILE",
    "fast" if STAGE4_FAST_MODE else "full_aux",
).strip().lower()
STAGE4_FULL_AUX_MODE = STAGE4_OUTPUT_PROFILE in ("full", "full_aux", "train", "complete")
STAGE4_FULL_AUX_EXPORT_MODE = STAGE4_OUTPUT_PROFILE in ("full_aux_export", "aux_export", "post_aux")
RECON_SUPPORT_COVERAGE_EXPAND_Q = float(os.environ.get("WIND_RECON_SUPPORT_COVERAGE_EXPAND_Q", "0.82"))
RECON_SUPPORT_COVERAGE_EXPAND_MIN = float(os.environ.get("WIND_RECON_SUPPORT_COVERAGE_EXPAND_MIN", "0.18"))
RECON_SUPPORT_COVERAGE_CONF_MIN = float(os.environ.get("WIND_RECON_SUPPORT_COVERAGE_CONF_MIN", "0.10"))
RECON_CONF_MID_BAND_BOOST = float(os.environ.get("WIND_RECON_CONF_MID_BAND_BOOST", "0.04"))
_stage4_quality_expand_default = "1" if (STAGE4_FULL_AUX_MODE or STAGE4_FULL_AUX_EXPORT_MODE) else "0"
STAGE4_ENABLE_QUALITY_EXPAND = os.environ.get("WIND_STAGE4_ENABLE_QUALITY_EXPAND", _stage4_quality_expand_default) not in ("0", "false", "False")
STAGE4_ENABLE_SUPPORT_FILL = os.environ.get("WIND_STAGE4_ENABLE_SUPPORT_FILL", "1") not in ("0", "false", "False")
STAGE4_ENABLE_TEMPORAL_FILL = os.environ.get("WIND_STAGE4_ENABLE_TEMPORAL_FILL", "1") not in ("0", "false", "False")
STAGE4_ENABLE_RELAX = os.environ.get("WIND_STAGE4_ENABLE_RELAX", "1") not in ("0", "false", "False")
STAGE4_ENABLE_PRUNE = os.environ.get("WIND_STAGE4_ENABLE_PRUNE", "1") not in ("0", "false", "False")
STAGE4_ENABLE_EXPAND = os.environ.get("WIND_STAGE4_ENABLE_EXPAND", "1") not in ("0", "false", "False")
STAGE4_ENABLE_DIRECT_ANCHOR_RESTORE = os.environ.get("WIND_STAGE4_ENABLE_DIRECT_ANCHOR_RESTORE", "1") not in ("0", "false", "False")
STAGE4_ENABLE_DIRECT_ANCHOR_FORCE = os.environ.get("WIND_STAGE4_ENABLE_DIRECT_ANCHOR_FORCE", "1") not in ("0", "false", "False")
STAGE4_GPU_MODE = os.environ.get("WIND_STAGE4_USE_GPU", "auto").strip().lower()
STAGE4_GPU_DEVICE = os.environ.get("WIND_STAGE4_GPU_DEVICE", "cuda:0").strip()
STAGE4_GPU_MIN_NUMEL = int(os.environ.get("WIND_STAGE4_GPU_MIN_NUMEL", "1200000"))


def _stage4_should_skip_post():
    if STAGE4_FULL_AUX_MODE or STAGE4_FULL_AUX_EXPORT_MODE:
        return False
    return STAGE4_FAST_MODE and STAGE4_FAST_SKIP_POST


def _stage4_should_skip_dense_aux():
    if STAGE4_FULL_AUX_MODE or STAGE4_FULL_AUX_EXPORT_MODE:
        return False
    return STAGE4_FAST_MODE and STAGE4_FAST_SKIP_DENSE_AUX


def _stage4_profile_support_keep_q():
    return 0.04 if (STAGE4_FULL_AUX_MODE or STAGE4_FULL_AUX_EXPORT_MODE) else 0.06


def _stage4_profile_expand_params():
    if STAGE4_FULL_AUX_MODE or STAGE4_FULL_AUX_EXPORT_MODE or STAGE4_ENABLE_QUALITY_EXPAND:
        return {
            "support_quantile": 0.82,
            "support_min": 0.18,
            "conf_base": 0.10,
            "conf_gain": 0.22,
            "max_fill_ratio": 0.08,
            "spread_limit_scale": 0.75,
        }
    return {
        "support_quantile": 0.88,
        "support_min": 0.18,
        "conf_base": 0.12,
        "conf_gain": 0.19,
        "max_fill_ratio": 0.05,
        "spread_limit_scale": 0.70,
    }


def _stage4_quality_profile_name():
    if STAGE4_FULL_AUX_MODE or STAGE4_FULL_AUX_EXPORT_MODE or STAGE4_ENABLE_QUALITY_EXPAND:
        return "aux_aggressive"
    return "fast_balanced"


def _resolve_stage4_gpu_device():
    if torch is None or F_torch is None:
        return None
    if STAGE4_GPU_MODE in ("0", "false", "off", "no", "cpu"):
        return None
    if not torch.cuda.is_available():
        return None
    try:
        dev = torch.device(STAGE4_GPU_DEVICE)
...
STAGE4_TORCH_DEVICE = _resolve_stage4_gpu_device()


def _stage4_gpu_enabled(shape=None):
    if STAGE4_TORCH_DEVICE is None:
        return False
    if shape is None:
        return True
    try:
        ...
        return False
    return numel >= max(1, STAGE4_GPU_MIN_NUMEL)


def _torch_from_numpy(arr):
    return torch.as_tensor(np.asarray(arr), dtype=torch.float32, device=STAGE4_TORCH_DEVICE)


def _torch_to_numpy(arr):
    return arr.detach().float().cpu().numpy()


def _torch_neighbor_avg(a_t: "torch.Tensor") -> "torch.Tensor":
    pad = F_torch.pad(a_t[None, None], (1, 1, 1, 1, 1, 1), mode="replicate")[0, 0]
    return (
...
def _torch_lap(a_t: "torch.Tensor") -> "torch.Tensor":
    out = torch.zeros_like(a_t)
    out[1:-1, 1:-1, 1:-1] = (
        a_t[1:-1, 1:-1, :-2] + a_t[1:-1, 1:-1, 2:] +
        a_t[1:-1, :-2, 1:-1] + a_t[1:-1, 2:, 1:-1] +
        a_t[:-2, 1:-1, 1:-1] + a_t[2:, 1:-1, 1:-1] -
        6.0 * a_t[1:-1, 1:-1, 1:-1]
    )
    return out
...
def _sanitize_observations(df, required_cols, source_name):
    ...
    speed_soft_penalty = None
    if {"u", "v"}.issubset(set(df.columns)):
        max_speed = float(cfg.MAX_WIND_SPEED_MS)
        soft_cap = max(max_speed, DIRECT_SOURCE_SOFT_SPEED_CAP_MS) if source_name in ("wind", "amdar", "turb") else max_speed
        speed_expr = ((pl.col("u").cast(pl.Float32, strict=False) ** 2 + pl.col("v").cast(pl.Float32, strict=False) ** 2).sqrt())
        df = df.filter(speed_expr <= soft_cap)
        if source_name in ("wind", "amdar", "turb"):
            speed_soft_penalty = (
                pl.when(speed_expr > max_speed)
                .then(pl.lit(DIRECT_SOURCE_SOFT_SPEED_PENALTY, dtype=pl.Float32))
    ...
    if speed_soft_penalty is not None:
        score = score * speed_soft_penalty
...
def _resolve_stage2_summary_path() -> str:
    return os.environ.get(
        "WIND_STAGE2_SUMMARY_PATH_OVERRIDE",
        os.path.join(cfg.BASE_DIR, "stage2_output", "stage2_summary.json"),
    )


def _resolve_stage3_input_dir() -> str:
    return os.environ.get(
        "WIND_STAGE3_INPUT_DIR",
        os.path.join(cfg.BASE_DIR, "stage3_output"),
    )


def _resolve_stage4_output_dir() -> str:
    return os.environ.get(
        "WIND_STAGE4_OUTPUT_DIR",
        os.path.join(cfg.BASE_DIR, os.environ.get("WIND_STAGE4_OUTPUT_DIRNAME", "stage4_output_v2")),
    )


def _resolve_stage4_aux_source_dir() -> str:
    return os.environ.get(
        "WIND_STAGE4_AUX_SOURCE_DIR",
        _resolve_stage4_output_dir(),
    )
...
def _build_where2comm_targets(
    ...
    if physics_weight is not None and np.shape(physics_weight) == np.shape(support_strength):
        physics_term = np.clip(np.asarray(physics_weight, dtype=np.float32), 0.0, 1.0)
    else:
        physics_term = np.zeros_like(support_strength, dtype=np.float32)
    if source_diversity is not None and np.shape(source_diversity) == np.shape(support_strength):
        diversity_term = np.clip(np.asarray(source_diversity, dtype=np.float32), 0.0, 1.0)
    else:
        diversity_term = np.zeros_like(support_strength, dtype=np.float32)
    if recon_conf.shape == support_strength.shape:
        uncertainty_score_3d = (
            support_strength
            * np.clip(1.0 - recon_conf, 0.0, 1.0)
            * np.clip(0.55 + 0.30 * physics_term + 0.15 * diversity_term, 0.0, 1.25)
        )
    else:
...
def _choose_wind_comm_source(wind_grouped_primary, amdar_grouped, wind_grouped_fallback):
    """给通信 wind branch 选择更稳的直接风候选。"""
    if wind_grouped_primary is not None and len(wind_grouped_primary) > 0:
        return wind_grouped_primary
    if amdar_grouped is not None and len(amdar_grouped) > 0:
        return amdar_grouped
    return wind_grouped_fallback
...
def _compute_pinn_proxy_fields(recon_u, recon_v, support_strength):
    ...
    - 误差分析指标
    """
    if _stage4_gpu_enabled(np.shape(recon_u)):
        u_t = _torch_from_numpy(recon_u)
        v_t = _torch_from_numpy(recon_v)
        support_t = _torch_from_numpy(support_strength)
        du_dx_t = torch.zeros_like(u_t)
        dv_dy_t = torch.zeros_like(v_t)
        du_dx_t[:, :, 1:-1] = 0.5 * (u_t[:, :, 2:] - u_t[:, :, :-2])
        dv_dy_t[:, 1:-1, :] = 0.5 * (v_t[:, 2:, :] - v_t[:, :-2, :])
        divergence_abs = _torch_to_numpy(torch.abs(du_dx_t + dv_dy_t))
        smoothness = _torch_to_numpy(torch.sqrt(_torch_lap(u_t) ** 2 + _torch_lap(v_t) ** 2))
        divergence_abs = divergence_abs.astype(np.float32, copy=False)
        smoothness = smoothness.astype(np.float32, copy=False)
    else:
        ...
        smoothness = np.sqrt(_torch_lap(_torch_from_numpy(recon_u)).cpu().numpy() ** 2 + _torch_lap(_torch_from_numpy(recon_v)).cpu().numpy() ** 2).astype(np.float32, copy=False) if _stage4_gpu_enabled(np.shape(recon_u)) else None
...
def _compute_source_diversity_field(source_fields):
    """把多源观测覆盖压缩成 3D 多样性场。"""
    source_keys = ["wind", "motion", "amdar", "turb", "seed"]
    stacked = np.stack(
        [np.clip(np.asarray(source_fields.get(k, 0.0), dtype=np.float32), 0.0, 1.0) for k in source_keys],
        axis=0,
    )
    ...
    entropy = -np.sum(
        np.where(mass > 0.0, mass * np.log(mass + 1e-8), 0.0),
        axis=0,
    )
    entropy = entropy / np.log(float(len(source_keys)))
    source_count = np.sum(stacked > 1e-6, axis=0).astype(np.float32, copy=False)
    source_count_norm = np.clip((source_count - 1.0) / max(1.0, len(source_keys) - 1.0), 0.0, 1.0)
    return np.clip(0.65 * entropy + 0.35 * source_count_norm, 0.0, 1.0).astype(np.float32, copy=False)
...
def _refine_recon_confidence(
    ...
    if _stage4_gpu_enabled(np.shape(u)):
        u_t = _torch_from_numpy(u)
        v_t = _torch_from_numpy(v)
        u_avg = _torch_to_numpy(_torch_neighbor_avg(u_t))
        v_avg = _torch_to_numpy(_torch_neighbor_avg(v_t))
    else:
    ...
    source_diversity = np.asarray(source_diversity_field, dtype=np.float32)
    support_conf = np.asarray(support_strength, dtype=np.float32)
    physics_consistency = np.clip(1.0 - local_residual, 0.0, 1.0)
    conf = np.clip(
        0.42 * conf +
        0.25 * source_conf +
        0.20 * support_conf +
        0.13 * source_diversity * physics_consistency,
        0.0,
        1.0,
    )
    conf = np.clip(conf * (1.0 - 0.30 * local_residual), 0.0, 1.0)
    ...
    direct_floor = np.clip(
        ...
        + 0.07 * source_diversity * direct_agreement
        + 0.05 * physics_consistency,
        0.35,
        0.98,
    )
    ...
    support_target = np.clip(
        ...
        + 0.36 * support_conf
        + 0.15 * source_conf
        + 0.10 * np.sqrt(np.clip(conf0, 0.0, 1.0))
        + 0.10 * source_diversity
        + 0.08 * physics_consistency,
        0.08,
        0.80,
    )
    temporal_target = np.clip(
        0.06 + 0.28 * support_conf + 0.10 * source_conf + 0.06 * source_diversity + 0.06 * physics_consistency,
        0.05,
        0.58,
    )
    indirect_mask = mask & (~direct_mask) & (~support_fill_mask) & (~temporal_fill_mask)
    indirect_target = np.clip(
        0.03 + 0.22 * support_conf + 0.06 * source_conf + 0.10 * source_diversity * physics_consistency,
        0.03,
        0.42,
    )
    if RECON_ENABLE_CONF_BOOSTS:
        support_target = np.clip(support_target + RECON_SUPPORT_CONF_BOOST * np.sqrt(np.clip(support_conf, 0.0, 1.0)), 0.08, 0.78)
        temporal_target = np.clip(
            temporal_target + RECON_TEMPORAL_CONF_BOOST * np.sqrt(np.clip(support_conf * physics_consistency, 0.0, 1.0)),
            0.05,
            0.62,
        )
        indirect_target = np.clip(
            indirect_target + RECON_INDIRECT_CONF_BOOST * np.sqrt(np.clip(support_conf * source_diversity, 0.0, 1.0)),
            0.03,
            0.46,
        )
    ...
    conf[relaxed_mask] *= (RECON_RELAX_KEEP_RATIO if RECON_ENABLE_CONF_BOOSTS else 0.92)
    mid_mask = mask & (~direct_mask) & (conf >= 0.10) & (conf <= 0.32)
    if STAGE4_ENABLE_QUALITY_EXPAND and np.any(mid_mask):
        conf[mid_mask] = np.clip(
            conf[mid_mask] + RECON_CONF_MID_BAND_BOOST * np.sqrt(np.clip(support_conf[mid_mask] * physics_consistency[mid_mask], 0.0, 1.0)),
            0.02,
            0.42,
        )
...
def _prune_low_quality_reconstruction(
    ...
    if positive_vals.size > 0:
        support_floor = max(support_floor, float(np.quantile(positive_vals, _stage4_profile_support_keep_q())))

    support_fill_keep_mask = support_fill_mask & (
        (support_positive >= max(support_floor, 0.10))
        & (conf >= max(RECON_CONF_KEEP_FLOOR - 0.02, 0.10))
    )
    temporal_fill_keep_mask = temporal_fill_mask & (conf >= max(RECON_CONF_KEEP_FLOOR - 0.02, 0.08))
    ...
    strong_conf_mask = conf >= max(RECON_CONF_KEEP_FLOOR + 0.07, 0.16)
...
def _expand_supported_reconstruction(
    recon_u,
    recon_v,
    recon_conf,
    recon_mask,
    ...
    """对中高 support 但被保守裁掉的局部区域做有限扩展。

    目标：
    - 轻微提升 coverage
    - 不碰直接锚点
    - 只在 support 足够、且局部已有稳定重构邻域时扩展
    """
    u = np.asarray(recon_u, dtype=np.float32)
    v = np.asarray(recon_v, dtype=np.float32)
    conf = np.asarray(recon_conf, dtype=np.float32)
    mask = np.asarray(recon_mask, dtype=np.float32)
    support = np.asarray(support_strength, dtype=np.float32)
    if not np.any(mask > 0):
        return u, v, conf, mask, 0

    pos = support[support > 0]
    if pos.size == 0:
        return u, v, conf, mask, 0
    support_thr = max(float(support_min), float(np.quantile(pos, float(support_quantile))))
    direct_mask = (
        np.asarray(source_fields["wind"] > 0, dtype=bool)
        | np.asarray(source_fields["amdar"] > 0, dtype=bool)
        | np.asarray(source_fields["turb"] > 0, dtype=bool)
    )
    candidate = (mask <= 0) & (support >= support_thr) & (~direct_mask)
    cand_idx = np.argwhere(candidate)
    if cand_idx.size == 0:
        return u, v, conf, mask, 0
    ...
    known_u = u[mask > 0]
    known_v = v[mask > 0]
    known_conf = conf[mask > 0]
    filled = 0
    limit = min(int(cand_idx.shape[0]), max(32, int(float(max_fill_ratio) * max(1, known_idx.shape[0]))))
    cand_scores = support[cand_idx[:, 0], cand_idx[:, 1], cand_idx[:, 2]]
    cand_idx = cand_idx[np.argsort(-cand_scores)]

    for zi, yi, xi in cand_idx[:limit]:
        delta = known_idx - np.array([zi, yi, xi], dtype=np.int32)
        gate = (
            (np.abs(delta[:, 0]) <= max(1, SUPPORT_RADIUS_Z + 1))
            & (np.abs(delta[:, 1]) <= max(2, SUPPORT_RADIUS_XY + 1))
            & (np.abs(delta[:, 2]) <= max(2, SUPPORT_RADIUS_XY + 1))
        )
        local = np.where(gate)[0]
        if local.size < 3:
            continue
        dist2 = np.sum(delta[local].astype(np.float32) ** 2, axis=1)
        order = np.argsort(dist2)[:8]
        local = local[order]
        dist2 = dist2[order]
        w = 1.0 / (dist2 + 1.0)
        w *= np.clip(known_conf[local], 0.08, 1.0)
        ws = float(np.sum(w))
        if ws <= 1e-8:
            continue
        fill_u = float(np.sum(known_u[local] * w) / ws)
        fill_v = float(np.sum(known_v[local] * w) / ws)
        local_spread = float(np.sqrt(np.sum(((known_u[local] - fill_u) ** 2 + (known_v[local] - fill_v) ** 2) * w) / ws))
        if local_spread > max(10.0, float(spread_limit_scale) * SUPPORT_FILL_MAX_LOCAL_SPREAD):
            continue
        u[zi, yi, xi] = fill_u
        v[zi, yi, xi] = fill_v
        conf[zi, yi, xi] = float(np.clip(max(RECON_SUPPORT_COVERAGE_CONF_MIN, float(conf_base) + float(conf_gain) * support[zi, yi, xi]), 0.10, 0.36))
        mask[zi, yi, xi] = 1.0
        filled += 1
    return u, v, conf, mask, filled
...
def _restore_direct_anchor_voxels(
    recon_u,
    recon_v,
    recon_conf,
    recon_mask,
    ...
    """兜底恢复被后处理误删的直接观测锚点。"""
    shape = np.asarray(recon_u).shape
    if len(shape) != 3:
        return 0
    z_dim, h_dim, w_dim = int(shape[0]), int(shape[1]), int(shape[2])
    source_map = _build_direct_source_maps(wind_grouped, amdar_grouped, turb_grouped)
    if not source_map:
        return 0
    ...
    for (zz, yy, xx), values in source_map.items():
        if not (0 <= zz < z_dim and 0 <= yy < h_dim and 0 <= xx < w_dim):
            continue
        if float(recon_mask[zz, yy, xx]) > 0.0:
            continue
        arr = np.asarray(values, dtype=np.float32)
        uv = arr[:, :2]
        w = np.clip(arr[:, 2], 0.05, 1.0)
        ...
        if ws <= 1e-8:
            continue
        mean_uv = np.sum(uv * w[:, None], axis=0) / ws
        mean_q = float(np.mean(w))
        recon_u[zz, yy, xx] = float(mean_uv[0])
        recon_v[zz, yy, xx] = float(mean_uv[1])
        recon_conf[zz, yy, xx] = float(np.clip(0.58 + 0.20 * mean_q, 0.55, 0.88))
        recon_mask[zz, yy, xx] = 1.0
        restored += 1
    return restored


def _enforce_direct_anchor_values(
    recon_u,
    recon_v,
    recon_conf,
    recon_mask,
    ...
    for (zz, yy, xx), values in source_map.items():
        ...
        if ws <= 1e-8:
            continue
        mean_uv = np.sum(uv * w[:, None], axis=0) / ws
        direct_speed = float(np.sqrt(mean_uv[0] ** 2 + mean_uv[1] ** 2))
        curr_u = float(recon_u[zz, yy, xx])
        curr_v = float(recon_v[zz, yy, xx])
        curr_speed = float(np.sqrt(curr_u * curr_u + curr_v * curr_v))
        diff = float(np.sqrt((curr_u - mean_uv[0]) ** 2 + (curr_v - mean_uv[1]) ** 2))
        should_force = (
            float(recon_mask[zz, yy, xx]) <= 0.0
            or (direct_speed >= 10.0 and curr_speed <= DIRECT_ANCHOR_ZERO_SPEED_MS)
            or (direct_speed >= 10.0 and diff >= DIRECT_ANCHOR_FORCE_DIFF_MS)
        )
        if not should_force:
            continue
        mean_q = float(np.mean(w))
        recon_u[zz, yy, xx] = float(mean_uv[0])
        recon_v[zz, yy, xx] = float(mean_uv[1])
        recon_conf[zz, yy, xx] = float(np.clip(max(float(recon_conf[zz, yy, xx]), 0.56 + 0.18 * mean_q), 0.50, 0.90))
        recon_mask[zz, yy, xx] = 1.0
        forced += 1
    return forced
...
def _dedupe_primary_wind_source(wind_grouped, amdar_grouped, turb_grouped):
    ...
    if overlap_ratio < 0.92:
        return wind_grouped, overlap_ratio, 0, 0, 0
    ...
    for zz, yy, xx, uu, vv in zip(z, y, x, u, v):
        ...
        if key in other_map:
            ...
            elif diff >= 0.35 * WIND_PRIMARY_CONFLICT_KEEP_MS:
                keep_this = True
                scale = 0.28
        else:
            keep_this = True
    ...
    fallback_count = 0
    if len(filtered) == 0 and STAGE4_ENABLE_PRIMARY_ANCHOR_FALLBACK:
        keep_n = min(
            max(1, PRIMARY_ANCHOR_FALLBACK_MAX),
            max(1, int(math.ceil(PRIMARY_ANCHOR_FALLBACK_RATIO * len(wind_grouped)))),
            len(wind_grouped),
        )
        sort_exprs = []
        if "qc_weight" in wind_grouped.columns:
            sort_exprs.append(pl.col("qc_weight").cast(pl.Float32, strict=False).fill_null(0.0))
        if "obs_conf" in wind_grouped.columns:
            sort_exprs.append(pl.col("obs_conf").cast(pl.Float32, strict=False).fill_null(0.0))
        if sort_exprs:
            fallback = wind_grouped.with_columns(
                sum(sort_exprs).alias("_fallback_score")
            ).sort("_fallback_score", descending=True).head(keep_n)
        else:
            fallback = wind_grouped.head(keep_n)
        if "_fallback_score" in fallback.columns:
            fallback = fallback.drop("_fallback_score")
        weight = float(np.clip(PRIMARY_ANCHOR_FALLBACK_WEIGHT, 0.0, 1.0))
        if len(fallback) > 0 and "qc_weight" in fallback.columns:
            fallback = fallback.with_columns((pl.col("qc_weight") * weight).clip(0.0, 1.0).alias("qc_weight"))
        if len(fallback) > 0 and "obs_conf" in fallback.columns:
            fallback = fallback.with_columns((pl.col("obs_conf") * weight).clip(0.0, 1.0).alias("obs_conf"))
        filtered = fallback
        fallback_count = int(len(filtered))
    removed = int(len(wind_grouped) - len(filtered))
    return filtered, overlap_ratio, removed, int(conflict_keep_count), fallback_count
...
def _dense_field_to_sparse(field, mask=None, fill_value=0.0):
    arr = np.asarray(field, dtype=np.float32)
    if mask is None:
        keep = np.isfinite(arr) & (np.abs(arr - float(fill_value)) > 1e-8)
    else:
        keep = np.asarray(mask, dtype=bool)
        if keep.shape != arr.shape:
            keep = np.broadcast_to(keep, arr.shape)
    idx = np.flatnonzero(keep.reshape(-1)).astype(np.uint32)
    if idx.size == 0:
        return idx, np.array([], dtype=np.float32)
    val = arr.reshape(-1)[idx].astype(np.float32, copy=False)
    return idx, val


def _inflate_local_field(local_arr, full_shape, bbox, fill_value=0.0, dtype=np.float32):
    full = np.full(full_shape, fill_value, dtype=dtype)
    z0, z1, y0, y1, x0, x1 = bbox
    full[z0:z1, y0:y1, x0:x1] = np.asarray(local_arr, dtype=dtype)
    return full


def _slice_prev_recon_state(prev_recon_state, bbox):
    if prev_recon_state is None:
        return None
    z0, z1, y0, y1, x0, x1 = bbox
    out = {"source_index": int(prev_recon_state.get("source_index", -1))}
    for key in ("recon_u", "recon_v", "recon_conf", "recon_mask"):
        arr = np.asarray(prev_recon_state.get(key, []), dtype=np.float32)
        if arr.ndim != 3:
            return None
        out[key] = arr[z0:z1, y0:y1, x0:x1].copy()
    return out


def _lift_where2comm_targets(local_targets, bbox, full_h_dim, full_w_dim):
    z0, z1, y0, y1, x0, x1 = bbox
    local_h_dim = max(1, int(y1 - y0))
    local_w_dim = max(1, int(x1 - x0))

    def _lift_idx(idx):
        idx = np.asarray(idx, dtype=np.uint32)
        if idx.size == 0:
            return np.array([], dtype=np.uint32)
        z, y, x = _linear_to_zyx(idx.astype(np.int64), local_h_dim, local_w_dim)
        return _zyx_to_linear_idx(z + z0, y + y0, x + x0, full_h_dim, full_w_dim).astype(np.uint32, copy=False)

    return {
        "wind_idx": _lift_idx(local_targets.get("wind_idx", [])),
        "wind_score": np.asarray(local_targets.get("wind_score", []), dtype=np.float32),
        "motion_idx": _lift_idx(local_targets.get("motion_idx", [])),
        "motion_score": np.asarray(local_targets.get("motion_score", []), dtype=np.float32),
        "uncertainty_idx": _lift_idx(local_targets.get("uncertainty_idx", [])),
        "uncertainty_score": np.asarray(local_targets.get("uncertainty_score", []), dtype=np.float32),
        "joint_idx": _lift_idx(local_targets.get("joint_idx", [])),
        "joint_score": np.asarray(local_targets.get("joint_score", []), dtype=np.float32),
    }
...
def _support_guided_fill(recon_u, recon_v, recon_conf, recon_mask, support_strength):
    ...
    for zi, yi, xi in candidate_idx[:max_fill]:
        ...
        recon_conf[zi, yi, xi] = float(
            np.clip(
                0.13 + 0.50 * support_val + 0.18 * coherence + 0.16 * mean_local_conf,
                0.10,
                0.82,
            )
        )
...
def _physics_guided_relaxation(recon_u, recon_v, recon_conf, recon_mask, support_strength):
    ...
    use_gpu = _stage4_gpu_enabled(np.shape(recon_u))
    for _ in range(steps):
        if use_gpu:
            u_avg = _torch_to_numpy(_torch_neighbor_avg(_torch_from_numpy(recon_u)))
            v_avg = _torch_to_numpy(_torch_neighbor_avg(_torch_from_numpy(recon_v)))
        else:
...
def _build_stage4_meta_from_summary(stage2_item, stage3_item):
    """仅用 summary 字段构建轻量触发元信息。

    目的：
    - 在真正读取大 npz、构建完整重构前，先判断当前帧是否值得做 Stage-4 重计算；
    - 对非触发帧直接走极简输出，避免把绝大多数时间花在无效重构上。
    """
    return {
        "source_index": int(stage2_item.get("source_index", -1)),
        "wind_voxels": int(stage2_item.get("wind_voxels", 0)),
        "motion_voxels": int(stage2_item.get("motion_voxels", 0)),
        "flight_ff_wind_edges": int(stage3_item.get("flight_ff_wind_edges", 0)),
        "flight_comm_allowed_agents": int(stage3_item.get("flight_comm_allowed_agents", 0)),
        # 使用 Stage-2/3 轻量统计构造一个 cheap proxy，供触发器前判使用。
        "recon_seed_strength": float(
            float(stage2_item.get("wind_voxels", 0))
            + 0.65 * float(stage2_item.get("motion_voxels", 0))
            + 1.2 * float(stage2_item.get("amdar_voxels", 0))
            + 0.9 * float(stage2_item.get("turb_voxels", 0))
...
def _prepare_frame(stage2_item, stage3_item, prev_recon_state=None):
    ...
    wind_grouped_primary, wind_overlap_ratio, wind_overlap_removed, wind_conflict_keep_count, wind_primary_fallback_count = _dedupe_primary_wind_source(
    ...
    trajectory_3d_full = _build_trajectory_volume(loc_grouped, cfg.Z_DIM, H_DIM, W_DIM)
    ...
    wind_comm_grouped = _choose_wind_comm_source(wind_grouped_primary, amdar_grouped, wind_grouped)

    support_strength_full = _build_support_strength(trajectory_3d_full, motion_grouped, flight_seed_df, cfg.Z_DIM, H_DIM, W_DIM)
    support_voxel_count = int(np.count_nonzero(support_strength_full > 0))

    # [性能说明] 触发帧不再在完整 3D 域上做 dense 计算，而是先根据 support+直接观测
    # 构造一个局部活动包围盒，只在该子域上重构，再回填到全尺寸数组。
    activity_mask = np.asarray(trajectory_3d_full > 0, dtype=bool)
    for df in (wind_grouped, motion_grouped, amdar_grouped, turb_grouped, flight_seed_df):
        if df is None or len(df) == 0 or not {"z", "y", "x"}.issubset(set(df.columns)):
            continue
        z = df["z"].cast(pl.Int32, strict=False).to_numpy()
        y = df["y"].cast(pl.Int32, strict=False).to_numpy()
        ...
        if np.any(keep):
            activity_mask[z[keep], y[keep], x[keep]] = True

    active_idx = np.argwhere(activity_mask)
    if active_idx.size == 0:
        z0, z1 = 0, cfg.Z_DIM
        y0, y1 = 0, H_DIM
        x0, x1 = 0, W_DIM
    else:
        pad_z = max(1, SUPPORT_RADIUS_Z + 1)
        pad_xy = max(2, SUPPORT_RADIUS_XY + 2)
        z0 = max(0, int(active_idx[:, 0].min()) - pad_z)
        z1 = min(cfg.Z_DIM, int(active_idx[:, 0].max()) + pad_z + 1)
        y0 = max(0, int(active_idx[:, 1].min()) - pad_xy)
        y1 = min(H_DIM, int(active_idx[:, 1].max()) + pad_xy + 1)
        x0 = max(0, int(active_idx[:, 2].min()) - pad_xy)
        x1 = min(W_DIM, int(active_idx[:, 2].max()) + pad_xy + 1)

    local_z_dim = int(max(1, z1 - z0))
    local_h_dim = int(max(1, y1 - y0))
    local_w_dim = int(max(1, x1 - x0))

    def _crop_df(df):
        if df is None or len(df) == 0 or not {"z", "y", "x"}.issubset(set(df.columns)):
            return pl.DataFrame()
        out = df.filter(
            (pl.col("z").cast(pl.Int32, strict=False) >= z0) & (pl.col("z").cast(pl.Int32, strict=False) < z1) &
            (pl.col("y").cast(pl.Int32, strict=False) >= y0) & (pl.col("y").cast(pl.Int32, strict=False) < y1) &
            (pl.col("x").cast(pl.Int32, strict=False) >= x0) & (pl.col("x").cast(pl.Int32, strict=False) < x1)
        )
        if len(out) == 0:
            return out
        return out.with_columns(
            (pl.col("z").cast(pl.Int32, strict=False) - z0).alias("z"),
            (pl.col("y").cast(pl.Int32, strict=False) - y0).alias("y"),
            (pl.col("x").cast(pl.Int32, strict=False) - x0).alias("x"),
        )

    wind_for_recon_local = _crop_df(wind_for_recon)
    motion_for_recon_local = _crop_df(motion_for_recon)
    amdar_grouped_local = _crop_df(amdar_grouped)
    turb_grouped_local = _crop_df(turb_grouped)
    wind_grouped_local = _crop_df(wind_grouped)
    wind_grouped_primary_local = _crop_df(wind_grouped_primary)
    motion_grouped_local = _crop_df(motion_grouped)
    loc_grouped_local = _crop_df(loc_grouped)
    flight_seed_df_local = _crop_df(flight_seed_df)
    wind_comm_grouped_local = _crop_df(wind_comm_grouped)
    trajectory_3d = np.asarray(trajectory_3d_full[z0:z1, y0:y1, x0:x1], dtype=np.float32)

    # Stage 4 的重构不是只靠图传播，而是“多源融合 + 退化兜底”。
    recon_u_local, recon_v_local, recon_conf_local, recon_mask_local = _reconstruct_wind_field(
        local_z_dim,
        local_h_dim,
        local_w_dim,
        wind_for_recon_local,
        motion_for_recon_local,
        amdar_grouped_local,
        turb_grouped_local,
    ...
    support_strength = _build_support_strength(trajectory_3d, motion_grouped_local, flight_seed_df_local, local_z_dim, local_h_dim, local_w_dim)
    source_fields = _build_confidence_source_fields(
        local_z_dim,
        local_h_dim,
        local_w_dim,
        wind_grouped=wind_grouped_primary_local,
        motion_grouped=motion_grouped_local,
        amdar_grouped=amdar_grouped_local,
        turb_grouped=turb_grouped_local,
        flight_seed_df=flight_seed_df_local,
    )
    source_diversity_3d = _compute_source_diversity_field(source_fields)
    ...
    recon_u = recon_u_local
    recon_v = recon_v_local
    recon_conf = recon_conf_local
    recon_mask = recon_mask_local
    mask_before_support = np.asarray(recon_mask > 0, dtype=bool).copy()
    support_fill_count = _support_guided_fill(recon_u, recon_v, recon_conf, recon_mask, support_strength) if STAGE4_ENABLE_SUPPORT_FILL else 0
    ...
    temporal_fill_count = _apply_temporal_background(
        ...
        recon_mask,
        support_strength,
        _slice_prev_recon_state(prev_recon_state, (z0, z1, y0, y1, x0, x1)),
        int(stage2_item.get("source_index", -1)),
    ) if STAGE4_ENABLE_TEMPORAL_FILL else 0
    ...
    base_recon_domain_mask = _build_domain_mask(
        ...
        grouped_frames=[wind_grouped_primary_local, motion_grouped_local, amdar_grouped_local, turb_grouped_local, flight_seed_df_local],
    )
    ...
    if recon_conf.size > 0:
        ...
        recon_conf = _refine_recon_confidence(
            ...
            source_diversity_field=source_diversity_3d,
        ...
        if STAGE4_ENABLE_EXPAND:
            expand_params = _stage4_profile_expand_params()
            recon_u, recon_v, recon_conf, recon_mask, support_expand_count = _expand_supported_reconstruction(
                recon_u=recon_u,
                recon_v=recon_v,
                ...
                support_strength=support_strength,
                source_fields=source_fields,
                support_quantile=expand_params["support_quantile"],
                support_min=expand_params["support_min"],
                conf_base=expand_params["conf_base"],
                conf_gain=expand_params["conf_gain"],
                max_fill_ratio=expand_params["max_fill_ratio"],
                spread_limit_scale=expand_params["spread_limit_scale"],
            )
        else:
            support_expand_count = 0
        anchor_restore_count = _restore_direct_anchor_voxels(
            recon_u=recon_u,
            recon_v=recon_v,
            recon_conf=recon_conf,
            recon_mask=recon_mask,
            ...
            amdar_grouped=amdar_grouped_local,
            turb_grouped=turb_grouped_local,
        ) if STAGE4_ENABLE_DIRECT_ANCHOR_RESTORE else 0
        anchor_force_count = _enforce_direct_anchor_values(
            recon_u=recon_u,
            recon_v=recon_v,
            recon_conf=recon_conf,
            recon_mask=recon_mask,
            wind_grouped=wind_grouped_local,
            amdar_grouped=amdar_grouped_local,
            turb_grouped=turb_grouped_local,
        ) if STAGE4_ENABLE_DIRECT_ANCHOR_FORCE else 0
    else:
    ...
    if _stage4_should_skip_post():
        pinn_divergence_3d = np.zeros_like(recon_u, dtype=np.float32)
        pinn_smoothness_3d = np.zeros_like(recon_u, dtype=np.float32)
        physics_weight_3d = np.clip(support_strength, 0.0, 1.0).astype(np.float32, copy=False)
        where2comm_targets = _build_where2comm_targets(
            wind_grouped=wind_comm_grouped_local,
            motion_grouped=motion_grouped_local,
            support_strength=support_strength,
            recon_conf=np.nan_to_num(recon_conf, nan=0.0),
            h_dim=local_h_dim,
            w_dim=local_w_dim,
            physics_weight=None,
            source_diversity=source_diversity_3d,
        )
        forecast_u_3d = np.asarray(recon_u, dtype=np.float32).copy()
        forecast_v_3d = np.asarray(recon_v, dtype=np.float32).copy()
        forecast_conf_3d = np.asarray(recon_conf, dtype=np.float32).copy()
        forecast_mask_3d = np.asarray(recon_mask, dtype=np.float32).copy()
        hazard_shear_3d = np.zeros_like(recon_u, dtype=np.float32)
        hazard_turbulence_3d = np.zeros_like(recon_u, dtype=np.float32)
        hazard_alert_mask_3d = np.zeros_like(recon_u, dtype=np.float32)
    else:
        ...
        forecast_u_3d, forecast_v_3d, forecast_conf_3d, forecast_mask_3d = _forecast_next_wind_field(
            ...
            prev_recon_state=_slice_prev_recon_state(prev_recon_state, (z0, z1, y0, y1, x0, x1)),
    ...
    full_shape = (cfg.Z_DIM, H_DIM, W_DIM)
    recon_u = _inflate_local_field(recon_u, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    recon_v = _inflate_local_field(recon_v, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    recon_conf = _inflate_local_field(recon_conf, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    recon_mask = _inflate_local_field(recon_mask, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    trajectory_3d = _inflate_local_field(trajectory_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    support_strength = _inflate_local_field(support_strength, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    source_diversity_3d = _inflate_local_field(source_diversity_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    direct_agreement_field = _inflate_local_field(direct_agreement_field, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    direct_source_count_field = _inflate_local_field(direct_source_count_field, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    recon_support_domain_mask = _inflate_local_field(recon_support_domain_mask.astype(np.float32), full_shape, bbox, fill_value=0.0, dtype=np.float32) > 0
    recon_domain_mask = _inflate_local_field(recon_domain_mask.astype(np.float32), full_shape, bbox, fill_value=0.0, dtype=np.float32) > 0
    pinn_divergence_3d = _inflate_local_field(pinn_divergence_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    pinn_smoothness_3d = _inflate_local_field(pinn_smoothness_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    physics_weight_3d = _inflate_local_field(physics_weight_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    if _stage4_should_skip_dense_aux():
        diffusion_condition_4d = None
    else:
        diffusion_condition_4d = np.zeros((7, cfg.Z_DIM, H_DIM, W_DIM), dtype=np.float32)
        diffusion_condition_4d[:, z0:z1, y0:y1, x0:x1] = diffusion_condition_4d[:, z0:z1, y0:y1, x0:x1] + _build_diffusion_condition_tensors(
            radar_img=radar_img[y0:y1, x0:x1],
            trajectory_3d=np.asarray(trajectory_3d[z0:z1, y0:y1, x0:x1], dtype=np.float32),
            recon_u=np.asarray(recon_u[z0:z1, y0:y1, x0:x1], dtype=np.float32),
            recon_v=np.asarray(recon_v[z0:z1, y0:y1, x0:x1], dtype=np.float32),
            recon_conf=np.asarray(recon_conf[z0:z1, y0:y1, x0:x1], dtype=np.float32),
            support_strength=np.asarray(support_strength[z0:z1, y0:y1, x0:x1], dtype=np.float32),
            physics_weight=np.asarray(physics_weight_3d[z0:z1, y0:y1, x0:x1], dtype=np.float32),
        )
    forecast_u_3d = _inflate_local_field(forecast_u_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    forecast_v_3d = _inflate_local_field(forecast_v_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    forecast_conf_3d = _inflate_local_field(forecast_conf_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    forecast_mask_3d = _inflate_local_field(forecast_mask_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    hazard_shear_3d = _inflate_local_field(hazard_shear_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    hazard_turbulence_3d = _inflate_local_field(hazard_turbulence_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    hazard_alert_mask_3d = _inflate_local_field(hazard_alert_mask_3d, full_shape, bbox, fill_value=0.0, dtype=np.float32)
    where2comm_targets = _lift_where2comm_targets(where2comm_targets, bbox, H_DIM, W_DIM)

    print(
        f"[Stage-4][diag] frame={stage3_item['time_str']} "
        f"wind_raw={len(wind_grouped_raw)} wind={len(wind_grouped)} "
        f"wind_primary={len(wind_grouped_primary)} overlap={wind_overlap_ratio:.3f} removed={wind_overlap_removed} "
        f"conflict_keep={wind_conflict_keep_count} primary_fallback={wind_primary_fallback_count} "
        ...
        f"support_expand={support_expand_count} "
        f"anchor_restore={anchor_restore_count} anchor_force={anchor_force_count} "
    ...
    return {
        ...
        "wind_primary_fallback_count": int(wind_primary_fallback_count),
        ...
        "support_expand_count": int(support_expand_count),
        "temporal_fill_count": temporal_fill_count,
        "temporal_fill_kept_count": temporal_fill_kept_count,
        "anchor_restore_count": int(anchor_restore_count),
        "anchor_force_count": int(anchor_force_count),
...
def _build_sparse_stage4_payload(frame, trigger, trigger_reason):
    """单次构建最终 Stage-4 npz payload，避免双重写盘。"""
    h_dim = int(frame["radar_img"].shape[0])
    w_dim = int(frame["radar_img"].shape[1])
    full_shape = (cfg.Z_DIM, h_dim, w_dim)

    wind_grouped = frame["wind_grouped"]
    loc_grouped = frame["loc_grouped"]
    motion_grouped = frame["motion_grouped"]
    amdar_grouped = frame["amdar_grouped"]
    turb_grouped = frame["turb_grouped"]
    flight_pack = frame["flight_pack"]

    if len(wind_grouped) > 0:
        uv_idx = _zyx_to_linear_idx(wind_grouped["z"].to_numpy(), wind_grouped["y"].to_numpy(), wind_grouped["x"].to_numpy(), h_dim, w_dim)
        u_val = wind_grouped["u"].to_numpy().astype(np.float32, copy=False)
        v_val = wind_grouped["v"].to_numpy().astype(np.float32, copy=False)
        wind_count_val = (
            wind_grouped["obs_count"].to_numpy().astype(np.float32, copy=False)
            if "obs_count" in wind_grouped.columns else np.ones(len(wind_grouped), dtype=np.float32)
        )
        wind_conf_val = (
            wind_grouped["obs_conf"].to_numpy().astype(np.float32, copy=False)
            if "obs_conf" in wind_grouped.columns else np.zeros(len(wind_grouped), dtype=np.float32)
        )
    else:
    ...
    if len(loc_grouped) > 0:
        traj_idx = _zyx_to_linear_idx(loc_grouped["z"].to_numpy(), loc_grouped["y"].to_numpy(), loc_grouped["x"].to_numpy(), h_dim, w_dim)
        traj_val = (
            loc_grouped["density"].to_numpy().astype(np.float32, copy=False)
            if "density" in loc_grouped.columns else np.ones(len(loc_grouped), dtype=np.float32)
        )
    else:
        traj_idx = np.array([], dtype=np.uint32)
        traj_val = np.array([], dtype=np.float32)

    if len(motion_grouped) > 0:
        motion_idx = _zyx_to_linear_idx(motion_grouped["z"].to_numpy(), motion_grouped["y"].to_numpy(), motion_grouped["x"].to_numpy(), h_dim, w_dim)
        motion_u_val = motion_grouped["u_motion"].to_numpy().astype(np.float32, copy=False)
        motion_v_val = motion_grouped["v_motion"].to_numpy().astype(np.float32, copy=False)
        motion_count_val = (
            motion_grouped["motion_count"].to_numpy().astype(np.float32, copy=False)
            if "motion_count" in motion_grouped.columns else np.ones(len(motion_grouped), dtype=np.float32)
        )
    else:
        motion_idx = np.array([], dtype=np.uint32)
    ...
    def _build_agent_sparse(grouped, u_col="u", v_col="v"):
        if len(grouped) == 0:
        ...
        return (
            idx,
            grouped[u_col].to_numpy().astype(np.float32, copy=False),
            grouped[v_col].to_numpy().astype(np.float32, copy=False),
        )

    amdar_idx, amdar_u_val, amdar_v_val = _build_agent_sparse(amdar_grouped)
    turb_idx, turb_u_val, turb_v_val = _build_agent_sparse(turb_grouped)

    targets = frame["where2comm_targets"]
    recon_u = np.nan_to_num(frame["recon_u"], nan=0.0).astype(np.float32, copy=False) if trigger and frame["recon_u"] is not None else np.zeros(full_shape, dtype=np.float32)
    recon_v = np.nan_to_num(frame["recon_v"], nan=0.0).astype(np.float32, copy=False) if trigger and frame["recon_v"] is not None else np.zeros(full_shape, dtype=np.float32)
    recon_conf = np.nan_to_num(frame["recon_conf"], nan=0.0).astype(np.float32, copy=False) if trigger and frame["recon_conf"] is not None else np.zeros(full_shape, dtype=np.float32)
    recon_mask = np.asarray(frame["recon_mask"], dtype=np.float32) if trigger and frame["recon_mask"] is not None else np.zeros(full_shape, dtype=np.float32)
    trajectory_3d = np.asarray(frame["trajectory_3d"], dtype=np.float32) if frame["trajectory_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    physics_weight_3d = np.asarray(frame["physics_weight_3d"], dtype=np.float32) if frame["physics_weight_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    source_diversity_3d = np.asarray(frame["source_diversity_3d"], dtype=np.float32) if frame["source_diversity_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    direct_agreement_3d = np.asarray(frame["direct_agreement_3d"], dtype=np.float32) if frame["direct_agreement_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    direct_source_count_3d = np.asarray(frame["direct_source_count_3d"], dtype=np.float32) if frame["direct_source_count_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    pinn_divergence_3d = np.asarray(frame["pinn_divergence_3d"], dtype=np.float32) if frame["pinn_divergence_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    pinn_smoothness_3d = np.asarray(frame["pinn_smoothness_3d"], dtype=np.float32) if frame["pinn_smoothness_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    diffusion_condition_4d = np.asarray(frame["diffusion_condition_4d"], dtype=np.float32) if frame["diffusion_condition_4d"] is not None else None
    forecast_u_3d = np.asarray(frame["forecast_u_3d"], dtype=np.float32) if trigger and frame["forecast_u_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    forecast_v_3d = np.asarray(frame["forecast_v_3d"], dtype=np.float32) if trigger and frame["forecast_v_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    forecast_conf_3d = np.asarray(frame["forecast_conf_3d"], dtype=np.float32) if trigger and frame["forecast_conf_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    forecast_mask_3d = np.asarray(frame["forecast_mask_3d"], dtype=np.float32) if trigger and frame["forecast_mask_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    hazard_shear_3d = np.asarray(frame["hazard_shear_3d"], dtype=np.float32) if trigger and frame["hazard_shear_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    hazard_turbulence_3d = np.asarray(frame["hazard_turbulence_3d"], dtype=np.float32) if trigger and frame["hazard_turbulence_3d"] is not None else np.zeros(full_shape, dtype=np.float32)
    hazard_alert_mask_3d = np.asarray(frame["hazard_alert_mask_3d"], dtype=np.float32) if trigger and frame["hazard_alert_mask_3d"] is not None else np.zeros(full_shape, dtype=np.float32)

    fast_sparse_aux = _stage4_should_skip_dense_aux()
    recon_idx, recon_u_val = _dense_field_to_sparse(recon_u, mask=recon_mask > 0)
    _, recon_v_val = _dense_field_to_sparse(recon_v, mask=recon_mask > 0)
    _, recon_conf_val = _dense_field_to_sparse(recon_conf, mask=recon_mask > 0)
    recon_mask_val = np.ones_like(recon_u_val, dtype=np.float32)

    payload = {
        "storage_mode": np.array("sparse_lossless"),
        "grid_shape": np.asarray((cfg.Z_DIM, h_dim, w_dim), dtype=np.int32),
        ...
        "comm_idx": np.asarray(targets["joint_idx"], dtype=np.uint32),
        "comm_score": np.asarray(targets["joint_score"], dtype=np.float32),
        ...
        "comm_wind_score": np.asarray(targets["wind_score"], dtype=np.float32),
        "comm_motion_idx": np.asarray(targets["motion_idx"], dtype=np.uint32),
        "comm_motion_score": np.asarray(targets["motion_score"], dtype=np.float32),
        "agent_export": np.array(True),
        "ground_agent_id": np.array("ground_radar_center"),
        "ground_agent_type": np.array("radar_mosaic_center"),
        "ground_agent_lat": np.array(float(frame["stage3"].get("ground_lat", 0.0)), dtype=np.float32),
        "ground_agent_lon": np.array(float(frame["stage3"].get("ground_lon", 0.0)), dtype=np.float32),
        "ground_agent_alt_m": np.array(float(frame["stage3"].get("ground_alt", 0.0)), dtype=np.float32),
        "amdar_idx": amdar_idx,
        ...
        "recon_idx": recon_idx,
        "recon_u_val": recon_u_val,
        "recon_v_val": recon_v_val,
        "recon_conf_val": recon_conf_val,
        "recon_mask_val": recon_mask_val,
        "recon_triggered": np.array(int(bool(trigger)), dtype=np.int32),
        "recon_trigger_reason": np.array(trigger_reason, dtype="<U256"),
        "flight_topk": np.asarray(flight_pack.get("flight_topk", [])),
        "flight_agent_ids": np.asarray(flight_pack.get("flight_agent_ids", [])),
        "flight_offsets": np.asarray(flight_pack.get("flight_offsets", []), dtype=np.int64),
        "flight_idx_flat": np.asarray(flight_pack.get("flight_idx_flat", [])),
        "flight_u_flat": np.asarray(flight_pack.get("flight_u_flat", []), dtype=np.float32),
        "flight_v_flat": np.asarray(flight_pack.get("flight_v_flat", []), dtype=np.float32),
        "flight_count_flat": np.asarray(flight_pack.get("flight_count_flat", []), dtype=np.float32),
        "flight_mask": np.asarray(flight_pack.get("flight_mask", []), dtype=np.uint8),
        "flight_time_gap_sec": np.asarray(flight_pack.get("flight_time_gap_sec", []), dtype=np.float32),
        "flight_space_hdist_km": np.asarray(flight_pack.get("flight_space_hdist_km", []), dtype=np.float32),
        "flight_space_vdist_m": np.asarray(flight_pack.get("flight_space_vdist_m", []), dtype=np.float32),
        "flight_time_conf": np.asarray(flight_pack.get("flight_time_conf", []), dtype=np.float32),
        "flight_space_conf": np.asarray(flight_pack.get("flight_space_conf", []), dtype=np.float32),
        "flight_time_likelihood": np.asarray(flight_pack.get("flight_time_likelihood", []), dtype=np.float32),
        "flight_space_likelihood": np.asarray(flight_pack.get("flight_space_likelihood", []), dtype=np.float32),
        "flight_st_conf": np.asarray(flight_pack.get("flight_st_conf", []), dtype=np.float32),
        "flight_st_likelihood": np.asarray(flight_pack.get("flight_st_likelihood", []), dtype=np.float32),
        "flight_comm_allowed": np.asarray(flight_pack.get("flight_comm_allowed", []), dtype=np.float32),
        "flight_comm_weight": np.asarray(flight_pack.get("flight_comm_weight", []), dtype=np.float32),
        "flight_intent": np.asarray(flight_pack.get("flight_intent", []), dtype=np.float32),
        "flight_has_wind_obs": np.asarray(flight_pack.get("flight_has_wind_obs", []), dtype=np.float32),
        "ff_time_gap_sec": np.asarray(flight_pack.get("ff_time_gap_sec", []), dtype=np.float32),
        "ff_space_hdist_km": np.asarray(flight_pack.get("ff_space_hdist_km", []), dtype=np.float32),
        "ff_space_vdist_m": np.asarray(flight_pack.get("ff_space_vdist_m", []), dtype=np.float32),
        "ff_time_conf": np.asarray(flight_pack.get("ff_time_conf", []), dtype=np.float32),
        "ff_space_conf": np.asarray(flight_pack.get("ff_space_conf", []), dtype=np.float32),
        "ff_time_likelihood": np.asarray(flight_pack.get("ff_time_likelihood", []), dtype=np.float32),
        "ff_space_likelihood": np.asarray(flight_pack.get("ff_space_likelihood", []), dtype=np.float32),
        "ff_st_conf": np.asarray(flight_pack.get("ff_st_conf", []), dtype=np.float32),
        "ff_st_likelihood": np.asarray(flight_pack.get("ff_st_likelihood", []), dtype=np.float32),
        ...
        "ff_motion_allowed": np.asarray(flight_pack.get("ff_motion_allowed", []), dtype=np.float32),
        "ff_motion_weight": np.asarray(flight_pack.get("ff_motion_weight", []), dtype=np.float32),
        "ff_wind_allowed": np.asarray(flight_pack.get("ff_wind_allowed", []), dtype=np.float32),
        "ff_wind_weight": np.asarray(flight_pack.get("ff_wind_weight", []), dtype=np.float32),
        "trajectory_3d": trajectory_3d,
        "physics_weight_3d": physics_weight_3d,
    }
    ...
    if fast_sparse_aux:
        payload.update(
            {
                "source_diversity_idx": _dense_field_to_sparse(source_diversity_3d)[0],
                "source_diversity_val": _dense_field_to_sparse(source_diversity_3d)[1],
                "direct_agreement_idx": _dense_field_to_sparse(direct_agreement_3d)[0],
                "direct_agreement_val": _dense_field_to_sparse(direct_agreement_3d)[1],
                "direct_source_count_idx": _dense_field_to_sparse(direct_source_count_3d)[0],
                "direct_source_count_val": _dense_field_to_sparse(direct_source_count_3d)[1],
                "pinn_prior_u_3d": recon_u,
                "pinn_prior_v_3d": recon_v,
                "pinn_prior_confidence_3d": recon_conf,
            }
        )
    else:
        payload.update(
            {
                ...
                "pinn_smoothness_3d": pinn_smoothness_3d,
                "source_diversity_3d": source_diversity_3d,
                "direct_agreement_3d": direct_agreement_3d,
                "direct_source_count_3d": direct_source_count_3d,
                "diffusion_condition_4d": diffusion_condition_4d if diffusion_condition_4d is not None else np.zeros((7, *full_shape), dtype=np.float32),
                "diffusion_prior_u_3d": recon_u,
                "diffusion_prior_v_3d": recon_v,
                "diffusion_prior_confidence_3d": recon_conf,
                "forecast_u_3d": forecast_u_3d,
                "forecast_v_3d": forecast_v_3d,
                "forecast_confidence_3d": forecast_conf_3d,
                "forecast_mask_3d": forecast_mask_3d,
                "hazard_shear_3d": hazard_shear_3d,
                "hazard_turbulence_3d": hazard_turbulence_3d,
                "hazard_alert_mask_3d": hazard_alert_mask_3d,
...
def _build_lightweight_frame(stage2_item, stage3_item):
    """为非触发帧构建极简 Stage-4 样本。
    ...
    不做：
    - 完整风场重构
    - support / temporal / forecast / hazard / diffusion 条件生成
    """
    vox = np.load(stage2_item["vox_path"], allow_pickle=True)
    H_DIM, W_DIM, radar_img = _infer_hw_from_vox(vox)
    flight_pack = {}
    ...
    wind_grouped = _sanitize_observations(_records_to_df(vox[STAGE2_WIND_RECORDS]), ["z", "y", "x", "u", "v"], "wind")
    loc_grouped = _sanitize_observations(_records_to_df(vox[STAGE2_LOC_RECORDS]), ["z", "y", "x", "density"], "motion")
    motion_grouped = _sanitize_observations(_records_to_df(vox[STAGE2_MOTION_RECORDS]), ["z", "y", "x", "u_motion", "v_motion"], "motion")
    amdar_grouped = _sanitize_observations(_records_to_df(vox[STAGE2_AMDAR_RECORDS]), ["z", "y", "x", "u", "v"], "amdar")
    turb_grouped = _sanitize_observations(_records_to_df(vox[STAGE2_TURB_RECORDS]), ["z", "y", "x", "u", "v"], "turb")
    ...
    return {
        ...
        "recon_conf": None,
        "recon_mask": None,
        "recon_seed_strength": float(_build_stage4_meta_from_summary(stage2_item, stage3_item)["recon_seed_strength"]),
        "recon_support_domain_count": 0,
        "recon_domain_count": 0,
        "support_voxel_count": 0,
        "support_fill_count": 0,
        "support_fill_kept_count": 0,
        "temporal_fill_count": 0,
        "temporal_fill_kept_count": 0,
        "anchor_restore_count": 0,
        "anchor_force_count": 0,
        "relax_steps_used": 0,
        "outlier_drop_count": 0,
        "low_quality_pruned_count": 0,
        "direct_agreement_mean": 0.0,
        "where2comm_targets": {
            "joint_idx": np.array([], dtype=np.uint32),
            "joint_score": np.array([], dtype=np.float32),
            "wind_idx": np.array([], dtype=np.uint32),
            "wind_score": np.array([], dtype=np.float32),
            "motion_idx": np.array([], dtype=np.uint32),
            "motion_score": np.array([], dtype=np.float32),
            "uncertainty_idx": np.array([], dtype=np.uint32),
            "uncertainty_score": np.array([], dtype=np.float32),
        },
        "pinn_divergence_3d": None,
        "pinn_smoothness_3d": None,
        "physics_weight_3d": None,
        "source_diversity_3d": None,
        "direct_agreement_3d": None,
        "direct_source_count_3d": None,
        "diffusion_condition_4d": None,
        "forecast_u_3d": None,
        "forecast_v_3d": None,
        "forecast_conf_3d": None,
        "forecast_mask_3d": None,
        "hazard_shear_3d": None,
        "hazard_turbulence_3d": None,
        "hazard_alert_mask_3d": None,
        "flight_seed_df": pl.DataFrame(),
        "stage3": stage3_item,
        "stage2": stage2_item,
    }


def _load_stage4_fast_payload(npz_path):
    if not os.path.exists(npz_path):
        raise FileNotFoundError(f"Missing Stage-4 fast npz for full_aux_export: {npz_path}")
    with np.load(npz_path, allow_pickle=True) as base_npz:
        return {k: base_npz[k] for k in base_npz.files}


def _full_aux_zero_volume(shape):
    return np.zeros(shape, dtype=np.float32)


def _build_full_aux_fields_from_fast(stage2_item, stage3_item, base_payload):
    """基于已有 fast npz 派生完整辅助场，不重跑主链重构。"""
    vox = np.load(stage2_item["vox_path"], allow_pickle=True)
    h_dim, w_dim, radar_img = _infer_hw_from_vox(vox)
    full_shape = (cfg.Z_DIM, h_dim, w_dim)

    recon_u = np.asarray(base_payload.get("recon_u_3d", _full_aux_zero_volume(full_shape)), dtype=np.float32)
    recon_v = np.asarray(base_payload.get("recon_v_3d", _full_aux_zero_volume(full_shape)), dtype=np.float32)
    recon_conf = np.asarray(base_payload.get("recon_confidence_3d", _full_aux_zero_volume(full_shape)), dtype=np.float32)
    recon_mask = np.asarray(base_payload.get("recon_mask_3d", _full_aux_zero_volume(full_shape)), dtype=np.float32)
    trajectory_3d = np.asarray(base_payload.get("trajectory_3d", _full_aux_zero_volume(full_shape)), dtype=np.float32)
    physics_weight_3d = np.asarray(base_payload.get("physics_weight_3d", _full_aux_zero_volume(full_shape)), dtype=np.float32)
    ...
    amdar_grouped = _sanitize_observations(_records_to_df(vox[STAGE2_AMDAR_RECORDS]), ["z", "y", "x", "u", "v"], "amdar")
    turb_grouped = _sanitize_observations(_records_to_df(vox[STAGE2_TURB_RECORDS]), ["z", "y", "x", "u", "v"], "turb")
    flight_motion_records = _sanitize_observations(_records_to_df(vox[STAGE2_FLIGHT_MOTION_RECORDS]), ["z", "y", "x", "u_motion", "v_motion"], "motion")
    motion_grouped = _merge_motion_views(motion_grouped, flight_motion_records)

    wind_grouped_primary, _, _, _, _ = _dedupe_primary_wind_source(wind_grouped, amdar_grouped, turb_grouped)
    wind_comm_grouped = _choose_wind_comm_source(wind_grouped_primary, amdar_grouped, wind_grouped)
    flight_seed_df = _flight_pack_to_seed_df(flight_pack, h_dim, w_dim)
    ...
    source_fields = _build_confidence_source_fields(
        ...
        h_dim,
        w_dim,
        wind_grouped=wind_grouped_primary,
        motion_grouped=motion_grouped,
        amdar_grouped=amdar_grouped,
        turb_grouped=turb_grouped,
        flight_seed_df=flight_seed_df,
    ...
    source_diversity_3d = _compute_source_diversity_field(source_fields)
    direct_agreement_3d, direct_source_count_3d = _build_direct_agreement_fields(
        cfg.Z_DIM,
        h_dim,
        w_dim,
        wind_grouped=wind_grouped_primary,
    ...
    if np.size(physics_weight_3d) == 0 or np.shape(physics_weight_3d) != full_shape or not np.any(physics_weight_3d > 0):
        pinn_divergence_3d, pinn_smoothness_3d, physics_weight_3d = _compute_pinn_proxy_fields(recon_u, recon_v, support_strength)
    else:
        pinn_divergence_3d, pinn_smoothness_3d, _ = _compute_pinn_proxy_fields(recon_u, recon_v, support_strength)
    where2comm_targets = _build_where2comm_targets(
        wind_grouped=wind_comm_grouped,
        motion_grouped=motion_grouped,
        support_strength=support_strength,
        recon_conf=np.nan_to_num(recon_conf, nan=0.0),
        h_dim=h_dim,
    ...
    diffusion_condition_4d = _build_diffusion_condition_tensors(
        radar_img=np.asarray(radar_img, dtype=np.float32),
        trajectory_3d=trajectory_3d,
        recon_u=recon_u,
        recon_v=recon_v,
        recon_conf=recon_conf,
    ...
    return {
        "pinn_divergence_3d": np.asarray(pinn_divergence_3d, dtype=np.float32),
        "pinn_smoothness_3d": np.asarray(pinn_smoothness_3d, dtype=np.float32),
        "source_diversity_3d": np.asarray(source_diversity_3d, dtype=np.float32),
        "direct_agreement_3d": np.asarray(direct_agreement_3d, dtype=np.float32),
        "direct_source_count_3d": np.asarray(direct_source_count_3d, dtype=np.float32),
        "diffusion_condition_4d": np.asarray(diffusion_condition_4d, dtype=np.float32),
        "diffusion_prior_u_3d": recon_u,
        "diffusion_prior_v_3d": recon_v,
        "diffusion_prior_confidence_3d": recon_conf,
        "forecast_u_3d": np.asarray(forecast_u_3d, dtype=np.float32),
        "forecast_v_3d": np.asarray(forecast_v_3d, dtype=np.float32),
        "forecast_confidence_3d": np.asarray(forecast_conf_3d, dtype=np.float32),
        "forecast_mask_3d": np.asarray(forecast_mask_3d, dtype=np.float32),
        "hazard_shear_3d": np.asarray(hazard_shear_3d, dtype=np.float32),
        "hazard_turbulence_3d": np.asarray(hazard_turbulence_3d, dtype=np.float32),
        "hazard_alert_mask_3d": np.asarray(hazard_alert_mask_3d, dtype=np.float32),
        "physics_weight_3d": np.asarray(physics_weight_3d, dtype=np.float32),
        "trajectory_3d": trajectory_3d,
        "where2comm_targets": where2comm_targets,
    }


def _augment_existing_stage4_npz_with_full_aux(stage2_item, stage3_item, src_npz_path, dst_npz_path):
    """基于已有 fast 输出补写完整辅助场，不重跑主链重构。"""
    payload = _load_stage4_fast_payload(src_npz_path)
    aux_payload = _build_full_aux_fields_from_fast(stage2_item, stage3_item, payload)
    payload.update(aux_payload)
    dst_dir = os.path.dirname(dst_npz_path)
    if dst_dir:
        os.makedirs(dst_dir, exist_ok=True)
    np.savez_compressed(dst_npz_path, **payload)
    return aux_payload


# [改动说明] main 中加入统一选帧、进度条、增强 summary 字段与训练诊断所需统计。
def main():
    stage4_dir = _resolve_stage4_output_dir()
    ...
    numexpr_threads = os.environ.get("NUMEXPR_NUM_THREADS", "auto")
    polars_threads = os.environ.get("POLARS_MAX_THREADS", "auto")

    print("[Stage-4] 读取 Stage-2/Stage-3 中间结果...")
    print(f"[Stage-4] fast_mode={int(STAGE4_FAST_MODE)}")
    print(
        f"[Stage-4] gpu_mode={STAGE4_GPU_MODE} "
        f"gpu_enabled={int(STAGE4_TORCH_DEVICE is not None)} "
        f"gpu_device={STAGE4_TORCH_DEVICE if STAGE4_TORCH_DEVICE is not None else 'cpu'} "
        f"save_compressed={int(STAGE4_SAVE_COMPRESSED)} "
        f"output_profile={STAGE4_OUTPUT_PROFILE} "
        f"quality_expand_enabled={int(STAGE4_ENABLE_QUALITY_EXPAND)} "
        f"quality_profile={_stage4_quality_profile_name()} "
        f"support_fill={int(STAGE4_ENABLE_SUPPORT_FILL)} "
        f"temporal_fill={int(STAGE4_ENABLE_TEMPORAL_FILL)} "
        f"relax={int(STAGE4_ENABLE_RELAX)} "
        f"prune={int(STAGE4_ENABLE_PRUNE)} "
        f"expand={int(STAGE4_ENABLE_EXPAND)} "
        f"anchor_restore={int(STAGE4_ENABLE_DIRECT_ANCHOR_RESTORE)} "
        f"anchor_force={int(STAGE4_ENABLE_DIRECT_ANCHOR_FORCE)} "
        f"omp_threads={omp_threads} "
        f"mkl_threads={mkl_threads} "
        f"numexpr_threads={numexpr_threads} "
        f"polars_threads={polars_threads} "
        f"gpu_min_numel={STAGE4_GPU_MIN_NUMEL}"
    )
    stage2_summary = _load_json(_resolve_stage2_summary_path())
    stage3_summary, stage3_map = _load_stage3_pack_map(_resolve_stage3_input_dir())
    ...
    existing_summary_path = os.path.join(stage4_dir, "stage4_summary.json")
    if STAGE4_FULL_AUX_EXPORT_MODE:
        aux_source_dir = _resolve_stage4_aux_source_dir()
        source_summary_path = os.path.join(aux_source_dir, "stage4_summary.json")
        if not os.path.exists(source_summary_path):
            raise FileNotFoundError(f"full_aux_export requires existing fast stage4_summary.json: {source_summary_path}")
        summary = _load_json(source_summary_path)
        summary_map = {str(x.get("time_str")): x for x in summary}
        print(f"[Stage-4] full_aux_export mode: source={aux_source_dir}, out={stage4_dir}, frames={len(summary)}")
        os.makedirs(stage4_dir, exist_ok=True)
        for i, stage2_item in enumerate(stage2_summary, 1):
            ...
            item = summary_map.get(stage2_item["time_str"])
            if item is None:
                raise KeyError(f"Missing existing Stage-4 summary for time_str={stage2_item['time_str']}")
            src_npz_path = os.path.join(aux_source_dir, item["filename"])
            dst_npz_path = os.path.join(stage4_dir, item["filename"])
            _augment_existing_stage4_npz_with_full_aux(stage2_item, stage3_item, src_npz_path, dst_npz_path)
            if i % progress_every == 0 or i == total_frames:
                _print_stage_progress("Stage-4-full_aux_export", i, total_frames, t_start, frame_item=stage2_item)
        with open(os.path.join(stage4_dir, "stage4_summary.json"), "w", encoding="utf-8") as f:
            json.dump(summary, f, ensure_ascii=False, indent=2)
        print("[Stage-4] full_aux_export 完成")
    ...
    for i, stage2_item in enumerate(stage2_summary, 1):
        ...
        curr_frame_meta = _build_stage4_meta_from_summary(stage2_item, stage3_item)

        # 在保存之前先判断本帧是否为“触发帧”。
        # 触发帧才允许输出完整风场重构结果；普通帧只保留轻量状态。
        trigger, trigger_reason = _should_trigger_reconstruction(curr_frame_meta, prev_frame_meta)
        frame = _prepare_frame(stage2_item, stage3_item, prev_recon_state=prev_recon_state) if trigger else _build_lightweight_frame(stage2_item, stage3_item)
        save_name = f"frame_{frame['time_str']}.npz"
        save_path = os.path.join(stage4_dir, save_name)
        recon_conf_mean = _masked_mean(frame["recon_conf"], frame["recon_mask"]) if trigger else 0.0
        recon_coverage_ratio = float(
            np.sum(frame["recon_mask"] > 0) /
            max(1, int(frame.get("recon_domain_count", 0)))
        ) if trigger else 0.0
        payload = _build_sparse_stage4_payload(frame, trigger, trigger_reason)
        if STAGE4_FAST_MODE and not STAGE4_SAVE_COMPRESSED:
        ...
        summary.append(
            {
                ...
                "wind_primary_fallback_voxels": int(frame.get("wind_primary_fallback_count", 0)),
                ...
                "support_expand_voxels": int(frame.get("support_expand_count", 0)),
                ...
                "anchor_restore_voxels": int(frame.get("anchor_restore_count", 0)),
                "anchor_force_voxels": int(frame.get("anchor_force_count", 0)),
                ...
                # 这些辅助物理场在某些 fast / fallback 分支里可能为空，
                # 这里统一用 np.asarray(... ) 的安全包装方式转成数组；若为 None，则回退成空数组或 0。
                # 这样可以避免 `np.mean(None)` 触发 TypeError，同时不影响已有重构逻辑。
                "pinn_div_mean": float(np.mean(np.asarray(frame["pinn_divergence_3d"], dtype=np.float32))) if frame.get("pinn_divergence_3d") is not None and np.size(np.asarray(frame["pinn_divergence_3d"])) else 0.0,
                "pinn_div_p90": float(np.quantile(np.asarray(frame["pinn_divergence_3d"], dtype=np.float32), 0.90)) if frame.get("pinn_divergence_3d") is not None and np.size(np.asarray(frame["pinn_divergence_3d"])) else 0.0,
                "pinn_smooth_mean": float(np.mean(np.asarray(frame["pinn_smoothness_3d"], dtype=np.float32))) if frame.get("pinn_smoothness_3d") is not None and np.size(np.asarray(frame["pinn_smoothness_3d"])) else 0.0,
                "physics_weight_mean": float(np.mean(np.asarray(frame["physics_weight_3d"], dtype=np.float32))) if frame.get("physics_weight_3d") is not None and np.size(np.asarray(frame["physics_weight_3d"])) else 0.0,
                "source_diversity_mean": float(np.mean(np.asarray(frame["source_diversity_3d"], dtype=np.float32))) if frame.get("source_diversity_3d") is not None and np.size(np.asarray(frame["source_diversity_3d"])) else 0.0,
                "forecast_conf_mean": float(np.mean(np.asarray(frame["forecast_conf_3d"], dtype=np.float32))) if frame.get("forecast_conf_3d") is not None and np.size(np.asarray(frame["forecast_conf_3d"])) else 0.0,
                "forecast_coverage_ratio": float(
                    np.sum(np.asarray(frame["forecast_mask_3d"], dtype=np.float32) > 0) /
                    max(
                        1,
                        int(frame.get("recon_domain_count", 0)),
                        int(np.sum(np.asarray(frame["forecast_mask_3d"], dtype=np.float32) > 0)),
                    )
                ) if frame.get("forecast_mask_3d") is not None and np.size(np.asarray(frame["forecast_mask_3d"])) else 0.0,
                "hazard_alert_voxels": int(np.sum(np.asarray(frame["hazard_alert_mask_3d"], dtype=np.float32) > 0)) if frame.get("hazard_alert_mask_3d") is not None else 0,
                "hazard_shear_mean": float(np.mean(np.asarray(frame["hazard_shear_3d"], dtype=np.float32))) if frame.get("hazard_shear_3d") is not None and np.size(np.asarray(frame["hazard_shear_3d"])) else 0.0,
                "hazard_turbulence_mean": float(np.mean(np.asarray(frame["hazard_turbulence_3d"], dtype=np.float32))) if frame.get("hazard_turbulence_3d") is not None and np.size(np.asarray(frame["hazard_turbulence_3d"])) else 0.0,
                ...
                "recon_conf_p10": _masked_quantile(frame["recon_conf"], frame["recon_mask"], 0.10) if trigger else 0.0,
                ...
                "recon_conf_spread_p10_p90": (
                    _masked_quantile(frame["recon_conf"], frame["recon_mask"], 0.90)
                    - _masked_quantile(frame["recon_conf"], frame["recon_mask"], 0.10)
                ) if trigger else 0.0,
