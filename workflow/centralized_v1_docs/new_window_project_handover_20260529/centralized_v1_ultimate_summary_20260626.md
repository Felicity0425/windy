...
| `P0-GFS` | 完成并补齐高层 | `178/178` unique source，`200/200` frame，`failed_count = 0`，当前 `21` 层、`1000..100 hPa`、顶层约 `15.80 km` |
| `P0-GFS / verify_gfs_background` | 完成 | `ready_for_s4_oi_diag = true`，`supports_12km_plus = true` |
...
#### 0.3.4 `GFS forecast` 已经成为下一阶段背景主线，但定位是 weak/diagnostic background
...
2. 当前最终提取层已补齐为 `1000 ... 100 hPa`，共 `21` 层。
3. 当前最高层约对应 `15.80 km`，`supports_12km_plus = true`。
4. `verify_gfs_background` 最终结果为 `ready_for_s4_oi_diag = true`。
5. 所有 frame shape 已稳定为 `(21, 81, 45)`，没有缺帧、坏帧或大面积 NaN。

当前最合理的判断是：

```text
这批 GFS 已足够支持 report-only S4-OI-DIAG
但诊断结果表明它更适合作为 weak/diagnostic background，
不适合直接进入 official OI blend
```

#### 0.3.5 `S4-OI-DIAG` 的最终方法和结果
...
```text
1. 不改 official recon_u/v/conf/mask
2. 只把 GFS 当作 report-only background x_b
3. 对 train observations 计算 innovation = observation - background
4. 对 strict holdout 点单独计算 background-vs-holdout 误差
5. 按 altitude / count / gap / speed / time_conf 等 strata 分层
```
...
因此这一步的真实意义不是“证明 GFS 可以直接让 official OI 变好”，而是：

```text
证明 GFS 现在已经是一个可对齐、可分层、可用于诊断的独立背景；
同时也证明它当前更适合 weak/diagnostic background 角色，而不是 strong official blend 角色。
```

#### 0.3.6 `12km+` 的业务口径判断已经量化
...
1. `12km+` 的确是当前整体误差最强的污染源。
2. 如果产品业务范围允许改成 `<=12km`，当前系统会显得明显更可接受。

但要特别强调：

```text
这不等于全高度 official 目标已经解决；
...
```text
1. 先读完已经生成的 GFS verify / OI-DIAG / altitude-cutoff 三份报告
