...
## 10. 2026-06-05 formal guardrail and display-filled visualization

Stage4 现在把“正式精度验证”和“展示层全图补色”分成两个产品面：

```text
official accuracy:
  recon_u/v/conf/mask
  point_eval_*.csv/json
  pairwise/checklist strict holdout

display/product footprint:
  stage4_display_u_3d
  stage4_display_v_3d
...
`stage4_display_*` 的规则是：`recon_mask` 内保持 `tp26_thr11_preserve`，低置信或无 claim 区域只用弱背景做
low-confidence display fill。这个字段用于全图可视化和 product footprint，不能拿来替换 official RMSE/MAE，
也不能说 CMA/GFS/ERA 成为 truth。

pairwise/checklist 的正式 guardrail 已加入 wind-scale 字段：
...
light/moderate wind 中若 relative_error_ratio > 2 且 delta_vector_error > 5 m/s，直接 FAIL
```
...
| comparison | result | 结论 |
| --- | --- | --- |
| tp26 existing vs rerun | PASS | 200 帧复跑无 metric drift。 |
| tp26 vs SRHA | FAIL | weighted RMSE、P95、P99、12km+ 和 floor10 relative error 均劣化。 |
| tp26 vs sparse-temporal CMA | FAIL | weighted RMSE、12km+、light wind RMSE/MAE、floor10 relative error 均劣化。 |

代表帧 display-filled 图和风速/风向差异表：
...
当前 13.64 m/s 是 local consistency / representativeness sigma，不是飞机风观测误差。
sigma 不能从 Stage4 strict-holdout RMSE/MAE 里扣。
